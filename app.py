...
def ph_has(ph, ag: str) -> bool:
    # ph may be pandas Series or dict
    try:
        return int(ph.get(ag, 0)) == 1
    except Exception:
        # fallback for Series direct indexing
        try:
            return int(ph[ag]) == 1
        except Exception:
            return False
...
# --------------------------------------------------------------------------
# Auto-resolution from discriminating cells already available
# FIXES:
# - Runs ONLY when len(best) >= 2 (multiple suspects)
# - For DOSAGE antigens: NEG rule-out requires homozygous discriminating cell
# - If POS & NEG discriminating exist => inconclusive (do not rule-out)
# --------------------------------------------------------------------------
...
def auto_refine_combo_using_discriminating(best: tuple, cells: list):
    """
    Returns: (new_best_tuple, supported_set, ruled_out_set, inconclusive_set, evidence_lines)
    """
    if not best or len(best) < 2:
        return best, set(), set(), set(), []
    ...
    for ag in best:
        ...
        neg_disc_all = [c for c in disc if c["react"] == 0]

        # Strong neg for dosage = homozygous only
        neg_disc_strong = []
        for c in neg_disc_all:
            ph = c["ph"]
            if ag in DOSAGE:
                if is_homozygous(ph, ag):
                    neg_disc_strong.append(c)
            else:
                neg_disc_strong.append(c)

        if pos_disc and neg_disc_strong:
            inconclusive.add(ag)
            labs_pos = ", ".join([c["label"] for c in pos_disc[:5]])
            labs_neg = ", ".join([c["label"] for c in neg_disc_strong[:5]])
            evidence.append(
                f"Auto check Anti-{ag}: BOTH POS & NEG discriminating cells found → INCONCLUSIVE (dosage/variation). "
                f"POS: {labs_pos} | NEG(homozygous if dosage): {labs_neg}"
            )
            continue

        if neg_disc_strong and not pos_disc:
            ruled_out.add(ag)
            labs = ", ".join([c["label"] for c in neg_disc_strong[:5]])
            if ag in DOSAGE:
                evidence.append(f"Auto Rule-out Anti-{ag}: discriminating HOMOZYGOUS cell(s) NEGATIVE → {labs}")
            else:
    ...
    return new_best, supported, ruled_out, inconclusive, evidence
        ...
        if not st.session_state.lot_p or not st.session_state.lot_s:
            ...
            if all_rx:
                ...
                if not best:
                    ...
                    refined_best, supported_by_disc, ruled_out_by_disc, inconclusive_by_disc, evidence_lines = auto_refine_combo_using_discriminating(
                        best, cells
                    )
                    ...
                    if not best:
                        ...
                        if supported_by_disc:
                            ...
                            if show_supported:
                                st.info("Supported by discriminating cells (needs confirmation): " + ", ".join([f"Anti-{a}" for a in show_supported]))

                        if inconclusive_by_disc:
                            show_incon = [a for a in inconclusive_by_disc if a in best]
                            if show_incon:
                                st.warning(
                                    "Discriminating cells show mixed signals (dosage/variation) — do NOT auto rule-out: "
                                    + ", ".join([f"Anti-{a}" for a in show_incon])
                                )

                        if resolved:
                            st.success("Resolved (pattern explained & separable): " + ", ".join([f"Anti-{a}" for a in resolved]))
                        if needs_work:
                            st.warning("Pattern suggests these, but NOT separable yet (DO NOT confirm): " + ", ".join([f"Anti-{a}" for a in needs_work]))
