...
with st.sidebar:
    ...
    if st.checkbox("Supervisor Login"):
        if st.text_input("Pwd", type="password") == "admin123":
            st.info("Edit the 1s and 0s below:")
...
# Input
c_input_L, c_input_R = st.columns([1, 2])
with c_input_L:
    st.subheader("1. Control")
    ac_val = st.radio("Auto Control (AC)", ["Negative", "Positive"], horizontal=True)
    if ac_val == "Positive":
        st.error("🚨 Stop! Positive AC.")
        st.info("Check: 1. WAIHA (IgG+), 2. CAS (C3d+), 3. Delayed Transfusion Reaction (DHTR).")
        st.stop()
...
if st.checkbox("🔴 Run Analysis"):
    ...
    for ag in antigens_order:
        for idx, score in input_scores_map.items():
            if score == 0:
                if can_rule_out(ag, p_rows[idx-1]):
                    ruled_out.add(ag)
                    break
    ...
    if not matches:
        ...
        for ab in matches:
            ...
            if passed:
                st.markdown(f"<div class='status-pass'>✅ <b>Anti-{ab}: Confirmed ({method}).</b> Stats: {p_pos} Pos / {p_neg} Neg cells. (p ≤ 0.05)</div>", unsafe_allow_html=True)
            else:
                allow_print = False
                st.markdown(f"<div class='status-fail'>🛑 <b>Anti-{ab}: RULE NOT MET!</b><br>Need <b>{3-p_pos if 3-p_pos>0 else 0} more Pos</b> and <b>{3-p_neg if 3-p_neg>0 else 0} more Neg</b> cells.</div>", unsafe_allow_html=True)

        # Extra Cells Assistant
        if not allow_print:
            st.markdown("### ➕ Add Selected Cells")
            with st.container(border=True):
                c1, c2, c3, c4 = st.columns(4)
                new_id = c1.text_input("Cell ID")
                new_res = c2.selectbox("Result", ["Neg","1+","2+"])
                col3_t = c3.empty()
                temp_pheno = {}
                for m in matches:
                     val = col3_t.radio(f"{m}?", ["Pos (+)", "Neg (0)"], key=f"r_{m}")
                     temp_pheno[m] = 1 if "Pos" in val else 0
                if c4.button("Add Cell"):
                     if new_id:
            ...
            if st.session_state.extra_cells:
                st.write("**Added:**", [x['src'] for x in st.session_state.extra_cells])

    # Report
    st.divider()
    if allow_print:
        if st.button("🖨️ Generate Official Report"):
            xtra_txt = "<br><b>Extra Cells Used:</b> " + ", ".join([f"{x['src']}({x['res_str']})" for x in st.session_state.extra_cells]) if st.session_state.extra_cells else ""
            ...
            <div class='print-only'>
                ...
                <div class='results-box'>
                    <table width="100%">
                        <tr><td><b>Patient:</b> {p_name}</td><td><b>MRN:</b> {p_mrn}</td></tr>
                        <tr><td><b>Tech:</b> {p_tech}</td><td><b>Date:</b> {p_date}</td></tr>
                    </table>
                    <hr>
                    <p style="font-size:16px;"><b>Conclusion:</b> Anti-{', '.join(matches)} Detected.</p>
                    <p><b>Validation:</b> Rule of Three Met (p ≤ 0.05). {xtra_txt}</p>
                    <div style="background:#eef; padding:5px; border:1px solid #ccf;"><b>Note:</b> Phenotype patient for {', '.join(matches)} (Must be Negative).</div>
                    <hr><br><br>
                    <table width="100%"><tr><td><b>Signature:</b> __________________</td><td><b>Verified By:</b> __________________</td></tr></table>
                </div>
                <div class='watermark-print'>System V18.0 | Haitham Ismail © 2025</div>
            </div>
